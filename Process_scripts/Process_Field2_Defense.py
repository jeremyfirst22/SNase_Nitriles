...
fig2.text(0.01, (top-bottom)/2+bottom, r"$(\sigma_{\text{SRF}}^2 + \sigma_{\text{PCF}}^2)^{\frac{1}{2}} \left ( \frac{k_BT}{e^- \rm{\AA}} \right ) $", ha='left', va='center',rotation='vertical') 
fig2.text((right-left)/2+left, 0.01, r"FWHM (cm$^{-1}$)", ha='center', va='bottom') 
...
#        ax.errorbar(srfDict[molec]+pcfDict[molec]*fudgeFactor,absMaxDict[molec][0],yerr=absMaxDict[molec][2],xerr=err,color=colorDict[molec],capsize=2) 
        ...
        ax2.scatter(fwhmDict[molec][0],std,color=colorDict[molec],label=molec) 
        ax2.errorbar(fwhmDict[molec][0],std,xerr=fwhmDict[molec][1],color=colorDict[molec])
...
slope, intercept, r_value, p_value, std_error = linregress(fwhmAccum,stdAccum) 
xs = np.linspace(np.min(fwhmAccum),np.max(fwhmAccum),100) 
