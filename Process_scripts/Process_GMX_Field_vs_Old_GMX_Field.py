...
for molec in molecList : 
    apbsField = 'SNase_%s/force_calc_ca/SNase_%s.protein_field.projected.xvg'%(molec,molec) 
...
plt.xlabel('Old GMX Field') 
plt.ylabel('GMX Field') 
plt.legend() 

plt.savefig('figures/gmx_pcf_vs_old_gmx_pcf.png',format='png') 
plt.close()

apbs, gmx = [],[]
for molec in molecList : 
    apbsField = 'SNase_%s/force_calc_ca/SNase_%s.solvent_rxn_field.projected.xvg'%(molec,molec) 
    gmxField = 'SNase_%s/force_calc/SNase_%s.solvent_rxn_field.projected.xvg'%(molec,molec)
...
plt.savefig('figures/gmx_srf_vs_old_gmx_srf.png',format='png') 
plt.close()
