...
with open('Exp_data/pKas.dat') as f : 
    for line in f : 
        if not line.startswith('#') : 
            key = line.split()[0] 
            absMax  = float(line.split()[1] ) 
            absMaxDict[key] = absMax
...
for field in ['total_field','external_field','solvent_rxn_field','protein_field' ] : 
    ...
    fig2.text(0.06,0.5, r"pKa", ha='center', va='center',rotation='vertical')
    ...
    #    data2[:,1] /=max
        ...
        absMaxAccum.append(absMaxDict[molec]) 

        ax2.scatter(meanField,absMaxDict[molec],color=nameToColorKeys[molec],marker=nameToShapeKeys[molec] ) 
    ...
    fig2.savefig('figures/%s_vs_pKa.png'%field,format='png') 
...
for molec in srfDict : 
    ...
    ax.scatter(srfDict[molec]+pcfDict[molec]*fudgeFactor,absMaxDict[molec],color=nameToColorKeys[molec],s=5.0,marker=nameToShapeKeys[molec]) 
    ax.errorbar(srfDict[molec]+pcfDict[molec]*fudgeFactor,absMaxDict[molec],xerr=err,color=nameToColorKeys[molec],capsize=2) 
    ax.scatter(srfDict[molec],absMaxDict[molec],color=nameToColorKeys[molec],alpha=0.25,s=5.0) 

    #ax2.scatter(err,absMaxDict[molec][1],color=nameToColorKeys[molec]) 

    srfAccum.append(srfDict[molec]) 
    pcfAccum.append(pcfDict[molec]) 
    freqAccum.append(absMaxDict[molec]) 
...
ax.set_ylabel(r"Experimental pKa")
fig.legend()

fig.savefig('figures/pcf+srf_vs_pKa.png',format='png',dpi=500) 
