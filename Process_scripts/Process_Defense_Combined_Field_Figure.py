...
hspace, wspace = 0.25, 0.20

plotArray = gridspec.GridSpec(1,2,wspace=wspace,hspace=hspace,left=left,right=right,bottom=bottom,top=top)
...
fig.text(0.01,top-(top-bottom)/2   , r"Probability (a.u.)             ",ha='left',va='center',rotation='vertical')
fig.text(0.51,bottom+(top-bottom)/2, r"Probability (a.u.)            ",ha='left',va='center',rotation='vertical')

fig.text(left +(right-left-wspace/2)/4,0.01 , r"SRF $(\frac{k_B T}{e^{-}\AA})$",ha='center',va='bottom')
...
for plotRow,field in enumerate(['solvent_rxn_field','protein_field']) : 
    ...
    innerHist = gridspec.GridSpecFromSubplotSpec(figRows,figCols,subplot_spec=plotArray[plotRow],wspace=0.1,hspace=0.00)
...
fig.savefig('figures/defense_combined_figure_fields.png',format='png') 
plt.close(fig)
