...
#        ax3 = axarr3[index%figRows,index/figRows]
        ...
        data[:,0] -= avg 
        ###  Match standard deviations using FWHM of A90X as reference (4.2 cm^-1)
        data[:,0] *= (-4.2/(2.35*std)) 
        ### Move average of forces to observed frequency
        data[:,0] += absMaxDict[molec][0]
        ...
        conversion =   150.0 
        if False : #molec == 'N118X' : 
            bandwidth = -conversion * ftlsDict["A58X"][0]
        else : 
            bandwidth = -conversion * ftlsDict[molec][0]
        
        ### Broaden histogram bins with gaussian
        xmin, xmax, xstep = 2140,2180,500
        ...
        ax.plot(xs,bindata,color=colorDict[molec] )
        ax.text(0.01,0.88,"%s"%molec,va='top',ha='left',color=colorDict[molec],transform=ax.transAxes)
