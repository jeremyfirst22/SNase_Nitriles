...
equilTime= 00
for field in ['rxn_field','coloumb_field'] : 
    index=0
    fieldAccum = [] 
    stdAccum = [] 
    absMaxAccum = [] 
    ...
    fig3, axarr = plt.subplots(figRows,figCols,sharex='col',sharey='row',figsize=(4.3,6) )
    ...
    for index,molec in enumerate(molecList) :
        ...
        timeFile = "SNase_%s/APBS_fixed/%s.out"%(molec,field)  
        ...
        file="data_fitting/APBS_%s_%s"%(molec,field)
        np.savetxt(file+".xvg",data)
        command = "~/normal_distribution/tiltAngle -f %s.xvg -n 20 -o %s.out -p %s.poly -g %s.gaus -t 5 --overwrite"%(file,file,file,file)
        os.system(command)

        datafile = "data_fitting/APBS_%s_%s.out"%(molec,field)
        datafile2= "data_fitting/APBS_%s_%s.poly"%(molec,field)
        ...
        ax3.scatter(time,timeData,s=0.5,color=nameToColorKeys[molec],marker='.')  
    ...
    #    data2[:,1] /=max
        ...
        ax.axvline(np.average(timeData),color=nameToColorKeys[molec],linestyle='--',linewidth=1) 
    ...
    fig.savefig('figures/APBS_%s.png'%field,format='png') 
    fig2.savefig('figures/APBS_%s_vs_peak.png'%field,format='png') 
    fig3.savefig('figures/APBS_time_vs_%s.png'%field,format='png') #,dpi=1500) 
...
fwhmAccum,errAccum = [],[]
#for fudgeFactor in np.linspace(0.0,2.00,11) : 
for fudgeFactor in [0.25] : 
...
fig.savefig('figures/APBS_pcf+srf_vs_peak.png',format='png',dpi=500) 
fig2.savefig('figures/APBS_fwhm_vs_std.png') 
